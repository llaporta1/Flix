...
const MyFriendsScreen = ({ navigateTo }) => {
  ...
  const checkRegisteredUsers = async (contactsWithEmail) => {
    try {
      ...
      // Loop through each contact and check if they are registered in Firestore
      for (let contact of contactsWithEmail) {
        ...
        // Query Firestore to see if the contact is registered (i.e., exists in the 'users' collection)
        const q = query(collection(firestore, 'users'), where('email', '==', email));
        const querySnapshot = await getDocs(q);
  
        if (!querySnapshot.empty) {
          // If the contact is registered, add them to the registeredContacts array
          const contactData = querySnapshot.docs[0].data();
          registeredContacts.push({
            id: querySnapshot.docs[0].id, // Firestore ID of the user
            fullName: contact.givenName + ' ' + contact.familyName,
            email: email,
            profileImageUri: contactData.profileImageUri || null, // Assuming profileImageUri exists in Firestore
          });
        }
      }
  
      // Update the state with only the registered contacts
  ...
  const handleInvite = (contact) => {
    const emailTo = contact.emailAddresses[0]?.email;  // Fetch the first email address from the contact
    if (!emailTo) {
      Alert.alert('No Email', 'This contact does not have a valid email address.');
      return;
    }

    // Use the native mail app to send an invite email
    const subject = 'Join me on FLIX!';
    const body = `
      Hey, I’m inviting you to join me on FLIX!
      It's a casual social media app where we can share daily photo dumps with each other.
      You can download it here: [Link to the app].
    `;
    ...
    Linking.openURL(mailtoUrl)
      .catch((error) => {
        ...
        Alert.alert('Error', 'Failed to send the invitation email. Please try again.');
      });
};
        ...
        {activeTab === 'addFriends' && (
          <View style={styles.inner}>
            ...
            {contacts.length > 0 ? (
              ...
              <Text style={styles.noResultsText}>No contacts on FLIX</Text>
            )}
          </View>
        )}
